...
void plotTrainData(Mat trainData, Mat labels, const float *error = NULL) {
  ...
  if (error != nullptr) {
    stringstream ss;
    ss << "Error: " << *error << "\%";
    putText(plot, ss.str(), Point(20, 512 - 40), FONT_HERSHEY_SIMPLEX, 0.75, Scalar(200, 200, 200), 1, LINE_AA);
  }
...
**/
vector<vector<float> > ExtractFeatures(const Mat &img, vector<int> *left = nullptr, vector<int> *top = NULL) {
 ...
 */
Mat removeLight(const Mat &img, Mat pattern) {
  Mat aux;
...
**/
bool readFolderAndExtractFeatures(const string &folder, int label, int num_for_test,
...
void trainAndTest() {
  ...
  readFolderAndExtractFeatures("../data/ring/arandela_%04d.pgm", 1, num_for_test, trainingData, responsesData, testData,
                               testResponsesData);
  // get and process the screw images
  readFolderAndExtractFeatures("../data/screw/tornillo_%04d.pgm", 2, num_for_test, trainingData, responsesData,
